...
export const PRIMARY_DOMAINS = [
  ...
  'labour.go.th',            // กรมสวัสดิการฯ และสำนักงานพื้นที่ (ครอบคลุม osh.labour.go.th)
  // ── dlpw.go.th · ถอดออก 2026-08-21 · ห้ามใส่กลับเข้ามาโดยไม่ทดสอบก่อน ──────
  //   dlpw.go.th      → ENOTFOUND (ไม่มี A record)
  //   www.dlpw.go.th  → ENOTFOUND เช่นกัน
  //   อาการเดียวกับ krisdika ด้านบนทุกประการ · โดเมนทั้งโดเมนไม่ resolve
  //   ปล่อยไว้ใน allowed_domains ได้แค่ทำให้ web_search เสียช่องค้นไปกับโดเมนที่ไม่มีอยู่จริง
  //   ตัวบทของกรมสวัสดิการฯ ยังเปิดได้ที่ labour.go.th (osh.labour.go.th) ซึ่งอยู่ในรายการแล้ว
  ...
  // กรมธุรกิจพลังงาน — น้ำมันเชื้อเพลิง/ก๊าซ (หมวด LB)
  // คงไว้ทั้งที่ fetch() ของเราดึงไฟล์จากโฮสต์นี้ไม่ได้ (ยืนยัน 2026-08-21):
  //   doeb.go.th / www.doeb.go.th → "Response does not match the HTTP/1.1 protocol"
  //   เซิร์ฟเวอร์หลัง Imperva ตอบมาผิดรูปแบบจน undici ปฏิเสธ · ล้มเร็ว (~1.3 วิ) ไม่ค้าง
  // ไม่ใส่ DEAD_SOURCES เพราะ web_search รันบนเครื่องของ Anthropic ไม่ใช่เครื่องเรา
  // มันยังอ่านหน้านี้ได้และคืน excerpt ที่ผ่านด่านหลักฐานได้จริง (เส้นทาง firstRoundComplete)
  // ใส่ DEAD_SOURCES เมื่อไร = ตัดคำตอบที่ใช้ได้เหล่านั้นทิ้งไปด้วย ซึ่งเสียมากกว่าได้
  'doeb.go.th',
...
// 45_000 → 20_000 (2026-08-21)
//
// ตัวเลขนี้คุมทั้งการรอ header และการอ่าน body · จุดที่มันสำคัญคือ body
// เพราะ header ที่ตอบมาแล้วแต่ไม่ใช่ PDF ถูกด่าน content-type ตัดทิ้งทันทีอยู่แล้ว ไม่ค้าง
// ที่กินงบได้จริงคือโฮสต์ที่ตอบ content-type เป็น application/pdf แล้ว body ค้างกลางทาง
// ซึ่ง 45 วิ = 18% ของงบ 255 วิ ต่อ 1 ที่อยู่ โดยได้ null กลับมาเหมือนกัน
//
// วัดจากไฟล์ที่ดึงได้จริง 3 ไฟล์ (ratchakitcha 26KB–85KB): 1.9 / 2.6 / 2.6 วิ
// เว็บหน่วยงานไทยที่ยังตอบอยู่ตอบภายใน ~13 วิทุกตัวที่ทดสอบ · 20 วิจึงเผื่อไว้เกินพอ
// ดึงไม่ทันไม่ใช่ความล้มเหลว — ผู้เรียกถอยไปใช้ผลจาก web_search ตามเดิม
...
- legal.labour.go.th เปิดไม่ได้ (ใบรับรอง SSL ไม่ผ่าน) ให้ใช้ osh.labour.go.th แทน
- dlpw.go.th ใช้ไม่ได้แล้ว โดเมนไม่มีอยู่จริง (DNS ไม่ resolve) ห้ามคืนที่อยู่ในโดเมนนี้
...
// ── นิยาม web_search ชุดเดียวของทั้งระบบ · ห้ามก๊อปไปเขียนซ้ำที่อื่น ──────────
//
// `web_search_20250305` → `web_search_20260209` (2026-08-21)
// ตัวใหม่มี dynamic filtering คือรันโค้ดกรองผลค้นให้ในตัว ก่อนส่งกลับมาให้โมเดลอ่าน
// ตรงกับงานนี้พอดี เพราะเราค้นในโดเมนราชการที่คืนหน้าสารบัญ/หน้ารวมลิงก์ปนมาเยอะ
//
// **ผูกกับรุ่นโมเดล** — ตัวนี้ใช้ได้เฉพาะ Opus 5/4.8/4.7/4.6 · Sonnet 5 · Sonnet 4.6
// MODEL ค่าเริ่มต้นคือ claude-sonnet-4-6 ซึ่งรองรับ · แต่ถ้าตั้ง ANTHROPIC_MODEL
// ทับเป็นรุ่นเก่ากว่านั้น คำขอจะถูกตีกลับ 400 ทั้งคำขอ ไม่ใช่แค่เครื่องมือไม่ทำงาน
// เปลี่ยน ANTHROPIC_MODEL เมื่อไร ต้องเช็คบรรทัดนี้ด้วยเสมอ
//
// ห้ามประกาศ code_execution เพิ่มคู่กับตัวนี้ — dynamic filtering รันโค้ดอยู่แล้วในตัว
// ใส่เข้าไปอีกชุดจะกลายเป็นสองสภาพแวดล้อมซ้อนกันแล้วโมเดลสับสน
