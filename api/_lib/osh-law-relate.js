...
// TRUSTED_DOMAINS ไม่ต้อง import แล้ว — รายการโดเมนติดอยู่ใน WEB_SEARCH_TOOL ที่เดียว
...
// โควตาที่กันไว้ให้ specific ก่อนเสมอ · anchor + pending หยิบได้แค่ส่วนที่เหลือจากนี้
//
// ทำไมต้องกัน: anchor หยิบก่อน และหยิบได้ถึง MAX_TOTAL_LOOKUPS ตัว ตัวบทที่อ้างลอย ๆ
// เยอะพอ (12 จุดไม่ใช่เรื่องแปลกในกฎกระทรวงฉบับเดียว) จะกินโควตาหมดตั้งแต่คลื่นแรก
// เหลือ 0 ให้ pending recheck และเหลือ 0 ให้ลูป specific ซึ่งมีเงื่อนไข budget > 0
// ผลคือ **การอ้างที่ระบุเลขมาตราชัดเจน — ตรงที่สุดและถูกที่สุด — ถูกการอ้างลอย ๆ เบียดตกทั้งหมด**
// ซึ่งกลับหัวลำดับความสำคัญที่ควรเป็น
//
// ไม่ลด MAX_ANCHOR_LOOKUPS ลงตรง ๆ เพราะตัวบทที่ไม่มี specific เลยยังควรได้ anchor เต็ม 12 เหมือนเดิม
// การกันแบบนี้จึงไม่ลดเพดานของใคร ตราบใดที่อีกฝั่งไม่มีของให้ทำ
const MIN_SPECIFIC_RESERVE = 6
  ...
  try{
    ...
    const ar = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        // ใช้ WEB_SEARCH_TOOL ชุดกลาง — เดิมก๊อปนิยามมาเขียนซ้ำตรงนี้ทั้งที่ import มาแล้ว
        // แล้วมันก็ต่างจากต้นฉบับจริง ๆ ตอนอัปเวอร์ชันเครื่องมือ (2026-08-21)
        // ซึ่งคือสิ่งที่หัวไฟล์ law-source.js เตือนไว้ว่าจะเกิดถ้าปล่อยให้มีสองชุด
...
export async function relateAndMerge(relatedLaws, mainReqs, parentName, deadlineAt = Infinity){
  ...
  // กันโควตาให้ specific ไว้ก่อนแจกให้ anchor/pending — ดูเหตุผลที่ MIN_SPECIFIC_RESERVE
  // กันเท่าที่มีของจริงเท่านั้น ไม่มี specific สักตัว = ไม่กัน anchor ได้เต็มเพดานเหมือนเดิม
  const reserve = Math.min(specificRefs.length, MIN_SPECIFIC_RESERVE)
  const shared = Math.max(0, budget - reserve)

  const anchorTake = enoughTime ? anchorRefs.slice(0, Math.min(MAX_ANCHOR_LOOKUPS, shared)) : []
  budget -= anchorTake.length

  const pendingTake = (enoughTime && budget - reserve > 0)
    ? pendingRefs.slice(0, Math.min(MAX_PENDING_RECHECK, budget - reserve))
    : []
  const pendingChecked = pendingTake.length
  budget -= pendingTake.length

  // แยก "ตกเพราะเวลา" ออกจาก "ตกเพราะโควตา" ตั้งแต่ตรงนี้ — สองอย่างนี้ผู้ใช้ต้องทำคนละอย่าง
  // เวลาไม่พอ = กดสรุปซ้ำแล้วดีขึ้น (cache ช่วย) · โควตาหมด = กดซ้ำก็ได้ผลเดิม ต้องเปิดเอง
  const anchorSkippedForTime = enoughTime ? 0 : anchorRefs.length
  const anchorLeftOver = anchorRefs.slice(anchorTake.length)   // ตกเพราะโควตา (เมื่อ enoughTime)
  ...
  let skippedForTime = anchorSkippedForTime
  // ลูปหยุดเพราะอะไร — ตัวนี้กำหนดข้อความที่ผู้ใช้จะเห็นกับ frontier ที่ยังค้างอยู่
  // ค่าเริ่มต้นคือ 'budget' เพราะถ้าลูปไม่ได้เริ่มเลย เหตุผลเดียวที่เป็นไปได้คือโควตาหมด
  // (frontier.length ว่างจะไม่มีอะไรให้รายงานอยู่แล้ว)
  let frontierWhy = 'budget'
  ...
  const related_laws = [
    ...
    ...frontier.map(r => ({ law_name: r.law_name || '', clause: r.clause || 'ทั้งฉบับ',
      ...
      // เหตุผลต้องตรงกับที่เกิดขึ้นจริง — เดิมเขียน "เวลาไม่พอ" ตายตัว ทั้งที่ลูปหยุดเพราะ
      // โควตาหมดก็เข้าทางนี้ได้ · ผู้ใช้เห็น "เวลาไม่พอ" แล้วกดสรุปซ้ำ ก็ได้ผลเดิมทุกครั้ง
      note: frontierWhy === 'time'
        ? 'ยังไม่ได้ดึง — เวลาในรอบนี้ไม่พอ ลองสรุปซ้ำอีกครั้งจะดึงต่อจาก cache ได้เร็วขึ้น'
        : 'ยังไม่ได้ดึง — เกินจำนวนที่ดึงได้ในรอบเดียว กดสรุปซ้ำก็ได้ผลเดิม ต้องเปิดตัวบทเอง',
      from_cache: false, req_count: 0 })),
    // การอ้างแบบ whole_law ที่ไม่ได้โควตา · เดิมหายเงียบทั้งก้อน ไม่โผล่ที่ไหนเลย
    // (answers มีแต่ anchorTake · pendingRefs ส่วนเกินยังได้ pendingAnswer แต่ anchor ไม่ได้อะไร)
    // ผู้ใช้จึงไม่มีทางรู้ว่ามีจุดที่ระบบไม่ได้ตามไปอ่านให้
    ...anchorLeftOver.map(r => ({ law_name: r.law_name || '', clause: 'ทั้งฉบับ',
      status: 'manual', ref_type: 'whole_law', depth: 1, via: '',
      source_url: '', resolved_text: '', confidence: '',
      anchor_question: r.anchor_question || '', for_section: r.for_section || '',
      note: enoughTime
        ? 'ยังไม่ได้ค้น — เกินจำนวนคำถามที่ค้นได้ในรอบเดียว ต้องเปิดตัวบทที่อ้างถึงเอง'
        : 'ยังไม่ได้ค้น — เวลาในรอบนี้ไม่พอ ลองสรุปซ้ำอีกครั้ง',
      from_cache: false, req_count: 0 }))
  ]
